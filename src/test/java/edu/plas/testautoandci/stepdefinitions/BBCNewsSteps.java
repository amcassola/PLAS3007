...
public class BBCNewsSteps {
    ...
    public void iClickOnTheMenuItem(String item) {
        WebElement menuItem = Driver.getWebDriver().findElement(By.xpath("//*[@class='navigation-wide-list']/li//span[text()='" + item + "']"));

        menuItem.click();
    }

    @Then("^the '(.*)' index is displayed$")
    public void theMenuItemIndexIsDisplayed(String mainMenuItem) {
        if ("Video".equals(mainMenuItem)) {
            ...
            String selectedMenu = Driver.getWebDriver()
                    .findElement(By.cssSelector(".navigation-wide-list li.selected a")).getText();
            assertEquals(mainMenuItem, selectedMenu);
        }
    }
    ...
    public void eachItemInTheMarketsMenuContainsThe(String marketMenuItem) {
        List<WebElement> markets = Driver.getWebDriver()
                .findElements(By.className("markets-index-promo__row"));
        for (WebElement market : markets) {
            switch (marketMenuItem) {
                ...
                case "arrow":
                    ...
                    break;
                case "percentage":
                    String percentage = market
                            .findElement(By.xpath("//td[@class='down' or @class='up']")).getText();
                    assertTrue("Market index should include the percentage up or down (%)!",
                            percentage.contains("%"));
                    break;
                default:
                    throw new IllegalArgumentException("Only index, arrow and percentage accepted");
            }
        }
    }
